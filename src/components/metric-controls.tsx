...
export function MetricControls({
  ...
  const CONFIG = {
    align: {
      order: ["left", "center", "right"] as const,
      icon: (align: string) =>
        align === "left" ? (
    ...
    size: {
      order: ["small", "medium", "large"] as const,
      display: { small: "S", medium: "M", large: "L" },
    },
    ...
    <div className="flex gap-2 flex-wrap">
              ...
              {availableMetrics.map((metric) => (
                <Toggle
                  ...
                  onPressedChange={(pressed) =>
                    toggleMetric(metric.key, pressed)
                  }
                  size="sm"
                  variant="outline"
      ...
      {onLabelSizeChange && (
        <CycleButton
          ...
          display={(size) =>
            CONFIG.size.display[size as keyof typeof CONFIG.size.display]
          }
        />
      )}
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="px-3">
              {CONFIG.font.labels[
                fontFamily as keyof typeof CONFIG.font.labels
              ] || "Font"}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-80" align="start" side="top">
      ...
      {onExport && (
        <Button
          ...
          size="sm"
          onClick={onExport}
          className="px-3 text-green-600 hover:text-green-700 hover:bg-green-50 ml-auto"
        >
